...
def reseed(seed):
    """
    Reseeds random number generators for reproducibility.
    """
    import random
    import numpy as np
    import torch
...
def im_mean():
    """
    Returns the mean pixel value for padding or filling operations.
    Returns:
        tuple: Mean pixel values for each channel.
    """
    return tuple(int(m * 255) for m in IMAGENET_MEAN)


class VOSDataset(Dataset):
    """
    Works for MOSE/DAVIS/YouTubeVOS/BL30K training
    For each sequence:
    - Pick three frames
    - Pick two objects
    ...
    def __getitem__(self, idx):
        ...
        cls_gt = np.zeros((self.num_frames, 384, 384), dtype=np.uint8)
        first_frame_gt = np.zeros((1, self.max_num_obj, 384, 384), dtype=np.uint8)
